...
PRIV_TIMER_CHECK:
    			...
    			CMP R5,#MPCORE_PRIV_TIMER_IRQ		//check if the interrupt ID is the same as the clock ID and handle accordingly
				BNE KEYS_CHECK
				BL PRIV_TIMER_ISR
				B EXIT_IRQ
    
KEYS_CHECK:
    			CMP R5,#KEYS_IRQ					//check if the interrupt ID is the same as the KEYs ID and handle accordingly
